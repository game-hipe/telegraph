...
class TelegraphApi:
    ...
    async def method(self, method: str, values: Any = None, path: str = ""):
        ...
        response = (
            await self.session.post(
                "https://api.{}/{}/{}".format(self.domain, method, path), data=values
            )
        ).json()
    ...
    async def upload_file(self, f: Union[PathLike[str], BinaryIO]):
        ...
        with FilesOpener(f) as files:
            response = (
                await self.session.post(
                    "https://{}/upload".format(self.domain), files=files
                )
            ).json()
...
class Telegraph:
    ...
    async def edit_account_info(
        ...
        :param author_url: Default profile link, opened when users click on the
                           ...
                           not necessarily to a Telegram profile or channels
        """
        return await self._telegraph.method(
            "editAccountInfo",
            values={
                "short_name": short_name,
                "author_name": author_name,
    ...
    async def get_page(
        ...
        """
        response = await self._telegraph.method(
            "getPage", path=path, values={"return_content": return_content}
        )
    ...
    async def get_account_info(self, fields: Union[List[str], None] = None):
    ...
    async def upload_file(self, f: Union[PathLike, BinaryIO]):
        """Upload file. NOT PART OF OFFICIAL API, USE AT YOUR OWN RISK
            Returns a list of dicts with `src` key.
            Allowed only .jpg, .jpeg, .png, .gif and .mp4 files.
